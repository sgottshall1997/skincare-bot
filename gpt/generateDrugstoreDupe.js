...
module.exports = async function generateDrugstoreDupe(openai, product) {
  ...
  const [intro, content, outro] = await Promise.all([
    ...
    gptPrompt(`Describe how "${product}" is a great drugstore dupe for a more expensive product in 200 words or less. Compare the key ingredients, performance, and results.`),
